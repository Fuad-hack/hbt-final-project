...
async function loadDashboardData() {
  try {
    // Load alerts count for sidebar badge
...
async function loadAlertsCount() {
  try {
    const data = await apiGet('/api/alerts?limit=1');
    state.unreadAlerts = data.unread || 0;
    updateAlertsBadge(state.unreadAlerts);
  } catch (error) {
...
function updateAlertsBadge(count) {
  const badge = document.querySelector('.nav-item[data-view="alerts"] .nav-badge');
  if (badge) {
...
function updateDashboardStats(stats) {
  // Update KPI cards
  if (stats.alerts) {
    document.querySelector('.kpi-card.critical .kpi-value').textContent = stats.alerts.critical || 0;
    document.querySelector('.kpi-card.warning .kpi-value').textContent = stats.alerts.high || 0;
  }
  
  if (stats.users) {
    document.querySelector('.kpi-card.neutral .kpi-value').textContent = stats.users.total || 247;
  }
  
  if (stats.anomalies) {
    document.querySelector('.kpi-card.danger .kpi-value').textContent = stats.anomalies.avg_score || 0.73;
  }
  
  // Update sidebar badge
  if (stats.alerts) {
    updateAlertsBadge(stats.alerts.unread || 0);
  }
}
