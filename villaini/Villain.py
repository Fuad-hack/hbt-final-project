...
parser.add_argument("-b", "--bind", default="0.0.0.0", help = "Bind HTTP/TCP services to a specific IP address (e.g., 127.0.0.1).")
parser.add_argument("-s", "--services", default="all", help = "Comma-separated list of services to enable (e.g., hoaxshell,tcp,smuggler). Default: all")
...
bind_address = args.bind if args.bind else '0.0.0.0'
...
Hoaxshell_Settings.bind_address = bind_address
Core_Server_Settings.bind_address = bind_address
TCP_Sock_Handler_Settings.bind_address = bind_address
File_Smuggler_Settings.bind_address = bind_address
...
class Completer(object):
    def __init__(self):
        ...
        self.main_command_arguments = ['payload', 'lhost', 'obfuscate', 'encode', 'constraint_mode', 'exec_outfile', 'domain']
    ...
    def complete(self, text, state):
        ...
        elif (main_cmd in ['reset']) and (line_buffer_list_len > 1) and (line_buffer_list[-1][0] not in ["/", "~"]):
...
def main():
            ...
            if res_status_code == 200:
                ...
                for filename in files:
                    try:
                        ...
                        print(f'\r[{ERR}] Failed to fetch file: {filename}. Installation may be corrupt.')
                    ...
                    if not local_signature or (local_signature != latest_signature):
                        ...
                        if not update_consent:              
                            consent = input(f'\r[{INFO}] Updates detected. Proceed? [y/n]: ').lower().strip()
                            if consent in ['y', 'yes']:
                                update_consent = True
                                Loading.active = True
                                Thread(target = Loading.animate, args = (f'[{INFO}] Updating',), name = 'loading_animation', daemon = True).start()
                            else:
                                break
                        if update_consent:
    ...
    # ------------------ SEÇİMLİ SERVİS MODULLARI ------------------ #
    
    # Reverse TCP Handler Yoxlanışı
    if "all" in enabled_services or "tcp" in enabled_services:
    ...
    # Hoaxshell Engine Yoxlanışı
    if "all" in enabled_services or "hoaxshell" in enabled_services:
        ''' Init Hoaxshell Engine '''
        initiate_hoax_server()
        global payload_engine, sessions_manager
        payload_engine = Payload_Generator()
        sessions_manager = Sessions_Manager()
        Hoaxshell.server_unique_id = core.return_server_uniq_id()

    # File Smuggler Yoxlanışı
    if "all" in enabled_services or "smuggler" in enabled_services:
        ''' Init File Smuggler '''
    ...
    while True:
        try:
            ...
            if cmd in core.requests.keys():
                ...
                print(f'{ATT} Commands "upload" and "exec" can now only be used with an active pseudo shell.')
                continue
            ...
            if cmd == 'help':                   
                if cmd_list_len == 1:
                    ...
                    PrompHelp.print_detailed(cmd_list[1]) if cmd_list[1] in PrompHelp.commands.keys() else print(f'Command {cmd_list[1]} does not exist.')
            elif cmd == 'id':
                        ...
                        if isinstance(alias, list):
                            ...
                            for session_id in sessions:
                                if Sessions_Manager.active_sessions[session_id]['alias'] == alias.strip():
                                    unique = False
                                    break
                            is_reserved = alias in ['Undefined', 'Active', 'Stable', 'Unstable']
                            is_session_id = alias in sessions
                ...
                if sessions_check[0]:
                    ...
                    if key in ['hostname', 'username']:
                        ...
                        print(f'Repair function not applicable on "{key}".')
                else:
                ...
                if sessions_check[0]:
                    ...
                    villain_cmd = {'data' : execution_object, 'quiet' : False}
                    if session_owner_id == core.return_server_uniq_id():
                        ...
                        try:
                            verified = input(f'\r[{WARN}] This session belongs to a sibling server. Proceed? [y/n]: ')
                        except:
                            print()
                            verified = None
                ...
                try:
                    chk = input('Proceed with purge? [y/n]: ')
                except:
                    print()
                    continue
                ...
                if len(cmd_list) == 2:
                    ...
                    if operation == 'pop':
                        try: redirector_id = int(cmd_list[2])
                        except:
                            redirector_id = False
                        ...
                        if redirector_id <= redirectors_len and redirector_id >= 0:
                            for i in range(0, redirectors_len):
                                if i == redirector_id:
                                    ...
                                    if shell_occupied:
                                        choice = input('Session occupied. Remove redirector? [Y/n]: ')
